...
use crate::{cli::Invoke, config::{clusters::auth::{AuthType, AuthTypeNames}, Context, FromUserInputForVariant}, error::cli::config::cluster::WritableClusterError};
...
impl Invoke for SetCluster {
    ...
    fn invoke(self, mut ctx: Context) -> error_stack::Result<(), WritableClusterError> {
        let Self { name, bootstrap_servers,  auth} = self;

        let cluster = ctx.clusters_mut()
